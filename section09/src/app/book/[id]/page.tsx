...
async function BookDetail({ bookId }: { bookId: string }) {
  ...
  return (
    <section>
      ...
      >
        <Image src={coverImgUrl} width={240} height={300} alt={`도서 ${title}의 이미지`} />
      </div>
...
async function ReivewList({ bookId }: { bookId: string }) {
  ...
  return (
    <section>
      {reviews.map((review) => (
        <ReviewItem key={`reivew-item-${review.id}`} {...review} />
      ))}
    </section>
  );
}
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  return (
    <div className={style.container}>
      ...
      <ReivewList bookId={(await params).id} />
    </div>
  );
}
